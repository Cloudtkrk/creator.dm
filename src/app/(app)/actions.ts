...
import { getLead, getTemplate, getUserByLoginId } from "@/lib/queries";
...
export async function saveDailyReports(fd: FormData) {
  ...
  if (accountIds.length === 0) back("/daily", "保存対象のアカウントがありません。", "err");
  ...
  const rows = accountIds.map((id) => {
    ...
    if (reply > sent) {
      ...
      back(
        `/daily?date=${date}${target ? `&user=${target}` : ""}`,
        `返信数が送付数を上回っています（アカウントID ${id}）。入力を確認してください。`,
        "err",
      );
    }
  ...
  // 管理者が他の運用者の日報を入力しているケースがあるため、対象者も引き継ぐ
  const target = str(fd, "target_user");
  back(
    `/daily?date=${date}${target ? `&user=${target}` : ""}`,
    `${date} の日報を保存しました。`,
...
export async function saveTemplate(fd: FormData) {
  ...
  if (!title || !body.trim()) {
    back("/templates", "タイトルと本文の両方を入力してください。", "err");
  }
  ...
  if (id) {
    ...
    back(`/templates/${id}`, `v${version} として保存しました。`);
  }
  ...
  back(`/templates/${Number(info.lastInsertRowid)}`, "文章を作成しました。");
}
...
export async function toggleTemplateActive(fd: FormData) {
  ...
  back("/templates", t.is_active ? "使用停止にしました。" : "使用中に戻しました。");
}
...
export async function restoreTemplateRevision(fd: FormData) {
  ...
  const rev = db
    ...
    | undefined;
  if (!rev) back(`/templates/${templateId}`, "履歴が見つかりません。", "err");
  ...
  back(`/templates/${templateId}`, `v${rev.version} の内容を復元しました。`);
}
...
export async function saveLead(fd: FormData) {
  ...
  if (!handle) back("/leads", "クリエイターのアカウントIDを入力してください。", "err");
...
export async function advanceLead(fd: FormData) {
  ...
  back("/leads", `@${lead.creator_handle} のステージを更新しました。`);
}
