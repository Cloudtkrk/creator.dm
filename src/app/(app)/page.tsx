...
export default async function Dashboard({
  ...
  const day = today();
  const monthTotals = totalsInRange(start, end, scope);
  const todayTotals = totalsInRange(day, day, scope);
  const leads = leadCounts(start, end, scope);
  const series = dailySeries(addDays(day, -29), day, scope);
  const alerts = computeAlerts(scope).filter((a) => a.level !== "info");
  ...
  const accounts = listAccounts(isAdmin ? undefined : user.id);
  const byAccount = totalsByAccount(start, end);

  const rewardTotal = isAdmin
    ? users.reduce((s, u) => s + computeReward(u.id, month).total, 0)
    : computeReward(user.id, month).total;

  return (
    <>
      <div className="page-head">
        <div>
          <h1>{isAdmin ? "全体ダッシュボード" : `${user.name} さんのダッシュボード`}</h1>
          <p>
      ...
      <div className="grid cols-4">
        ...
        />
        <Stat
          label={isAdmin ? "今月の報酬合計（見込）" : "今月の報酬（見込）"}
          value={yen(rewardTotal)}
          sub={isAdmin ? `運用者 ${users.length}名の合計` : "確定前の概算です"}
        />
      </div>
